...
@section('content')
    ...
    <ul class="page-breadcrumb breadcrumb">
        ...
        <li>
            <a href="{{ route('keluarga.gallery.manage') }}">Gallery</a>
            <i class="fa fa-circle"></i>
        </li>
        <li>
            ...
            <div class="portlet light portlet-fit portlet-datatable bordered">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-green sbold uppercase">Manage Gallery</span>
                    </div>
                    <div class="actions">
                        <div class="btn-group">
                    ...
                    <div class="table-container">
                        <table class="table table-striped table-bordered table-hover" id="sample_3">
                            ...
                            @foreach($model as $row)
                                <tr>
                                    <td> {{ $no }} </td>
                                    <td> <img src="{{ url('assets/gallery/'.$row->img) }}" class="img-responsive img-thumbnail" style="max-width: 200px; max-height: 150px;"></td>
                                    <td> {{ $row->anggota->user->name }} </td>
                                    <td> {{ $row->views }} </td>
                                    <td> {{ date('d F Y',strtotime($row->created_at)) }} </td>
                                    <td class="center" width="130">
                                        <a href="{{ route('keluarga.gallery.delete',$row->id) }}" class="btn red-mint btn-xs"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php $no++ ?>
                            @endforeach
